...
- 수집 실패는 중단하지 않는다. 실패 회차는 집계에서 걸러지고, 브리핑이
  "당일 수집 실패로 전일 데이터 기준"을 첫 줄에 명시해 발행한다
- 집계·탐지 실패 시에는 이후 단계 중단 (데이터 파일이 깨진 상태로 분석 금지)
...
STEPS = [
    ("수집", "collect.py", False),     # (이름, 스크립트, 실패 시 중단 여부)
